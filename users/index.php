...
if($error_header!=404){ SEO('users','','','',$link); } else { SEO('0','','','',$link); }
...
if (( isset($_GET["a"]))and($_GET["a"]=='add'))
{
    echo'<script type="text/javascript">
        $(function (){
            alert_message(\'ok\',\'Новый сотрудник добавлен\');
            });
</script>';
...
<?
	  ...
	  include_once $url_system.'template/top_users.php';
...
echo'<div class="okss"><span class="title_book yop_booking"><i class="smena_">'.$row__221["kol"].'</i><span class="smena_1">'.PadejNumber($row__221["kol"],'Сотрудник,Сотрудника,Сотрудников').'</span></span></div>';
...
//забронировано						 
               ...
               echo'<div class="users_block users_block_2020 '.$class_party.'" op_rel="'.$row_8["id"].'">';
...
echo'<div class="name-user-57"><a href="users/'.$row_8["id"].'/" class="h57-2020"><span>'.$row_8["name_user"].'</span>'.$online.'';
...
echo'</div>';
		...
		$result_status22=mysql_time_query($link,'SELECT count(a.id) as cc FROM trips AS a WHERE a.visible=1 and a.id_user="'.htmlspecialchars(trim($row_8["id"])).'"');
               ...
               $result_status22=mysql_time_query($link,'SELECT count(a.id) as cc FROM k_clients AS a WHERE a.visible=1 and a.id_user="'.htmlspecialchars(trim($row_8["id"])).'"');
               if($result_status22->num_rows!=0)
               {
...
echo'<div class="booking_div da_book1"><div class="not_booling"></div><span class="h4" style="width:calc(100% - 60px)"><span>Работники не найдены.</span></span></div>';
