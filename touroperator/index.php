...
if($error_header!=404){ SEO('touroperator','','','',$link); } else { SEO('0','','','',$link); }
...
<?
	  ...
	  include_once $url_system.'template/top_touroperator.php';
...
echo'<div class="okss"><span class="title_book yop_booking"><i class="smena_">'.$row__221["kol"].'</i><span class="smena_1">'.PadejNumber($row__221["kol"],'туроператор,туроператора,туроператоров').'</span></span></div>';
...
echo'<div class="operator_block" op_rel="'.$row_8["id"].'">';
						 
echo'<div class="open_operator"></div>';						 

echo'<a href="touroperator/'.$row_8["id"].'/" class="h57"><span>'.$row_8["name"].'</span>';
...
echo'<div id="copy'.$copy.'" data-tooltip="Логин" data-clipboard-action="copy" data-clipboard-target="#copy'.$copy.'"  class="login_o  btn">'.$row_8["login"].'</div>';
						 $copy++;
echo'<div id="copy'.$copy.'" data-tooltip="Пароль" data-clipboard-action="copy" data-clipboard-target="#copy'.$copy.'" class="pass_o  btn">'.$row_8["password"].'</div>';	
						 $copy++;
echo'<div class="tell_o" data-tooltip="Телефон">'.$row_8["phone"].'</div>';
						 

if(($sign_admin==1)and($row_status22["cc"]==0))
{
echo'<div class="font-ranks del_operator_" data-tooltip="Удалить" id_rel="'.$row_8["id"].'"><span class="font-ranks-inner">x</span></div>';
}	else
{
...
echo'<a href="'.$row_8["url"].'" target="_blank" class="link_o rois">'.$row_8["url"].'</a>';						 
echo'<div class="comment_o rois">'.$row_8["comment"].'</div>';
...
echo'<div class="booking_div da_book1"><div class="not_booling"></div><span class="h4"><span>Туроператоры не найдены.</span></span></div>';
