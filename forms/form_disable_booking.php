...
$result_t=mysql_time_query($link,'Select b.id,b.id_user,b.status,b.id_object from booking as b where b.id="'.htmlspecialchars(trim($_GET['id'])).'"');
...
$token=token_access_compile($_GET['id'],'disable_booking_xx',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Отменить покупку по заявке</span></h1>'; ?>
...
echo'<div class="comme">Вы точно хотите отменить покупку по заявке <b>№'.$_GET["id"].'</b>?</div>';		
?>	
...
<div id="button_dell_booking_dis" class="save_button"><i>Выполнить</i></div>
