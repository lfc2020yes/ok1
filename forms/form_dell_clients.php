...
$result_t=mysql_time_query($link,'Select b.id,b.id_user,b.fio,b.code from k_clients as b where b.id="'.htmlspecialchars(trim($_GET['id'])).'"');
...
$token=token_access_compile($_GET['id'],'dell_clients_xx',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Удаление клиента</span></h1>'; ?>
...
*/
echo'<div class="comme">Вы точно хотите удалить клиента - '.$row_score["fio"].' <b>code - '.$row_score["code"].'</b>?</div>';		
?>	
...
<div id="button_dell_clients" class="save_button"><i>Удалить</i></div>
