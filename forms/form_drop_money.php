...
$status_admin=0;
$result_uu = mysql_time_query($link, 'select a.all_comission,a.paid_comission,b.name_user,a.block_comission from affiliates as a,r_user as b where b.id_role="7" and b.id=a.id_users and a.id_users="'.ht($id_user).'" ');
...
$token=token_access_compile($id_user,'bt_affiliates_money',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div id="Modal-one" class="box-modal table-modal eddd1 client_window"><div class="box-modal-pading">
        <div class="top_modal">
            ...
            <?
            echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($id_user)).'"><span>Запрос на вывод средств</span>
...
<span class="forms_dop_pa">'.$row_uu['name_user'].'</span>
        ...
        <div class="center_modal">
            <?
            echo'<form class="js-form-pay-money" id="vino_xd_affiliates_money" style=" padding:0; margin:0;" method="post" enctype="multipart/form-data">';
...
$(\'.tyyo\').trigger(\'keyup\');
            ...
            if($status_admin==0)
            {
                $query_string.='<div class="center_vert right_task_ccb" style="width: 100%; margin-top:20px;"><div class="add_cff_fin js-add-buy-money-but-x">Запросить</div>';
            }
