...
$window_style=0;
    ...
    //проверяем вдруг у заявке до этого есть оплаты
	if($row_score["status"]!=6)
	{
		//еще не выбрали как будет оплачивать клиент все или частично
		$window_style=1;
	} else
	{
	   //смотрим вдруг он уже ничего не должен и это последний шаг покупки
	   ...
	   //всего отдал
	   $result_status22=mysql_time_query($link,'SELECT sum(a.prepaid) as prepaid FROM booking_status_history AS a WHERE a.id_booking="'.htmlspecialchars(trim($_GET['id'])).'" and a.status=6');	
...
//внесение 2-3 или последнего аванса
if($window_style==3)
{
  $token=token_access_compile($_GET['id'],'bt_booking_avans_two',$secret);
	?>
...
echo'<div class="comme" style="margin-bottom:20px;">Укажите точную сумму внесенную клиентом.</div>';
	...
	$result_status21=mysql_time_query($link,'SELECT sum(a.commission) as com, sum(a.cost) as cost FROM booking_offers AS a WHERE a.id_booking="'.htmlspecialchars(trim($_GET['id'])).'" and a.status=2');	
		...
		$result_status22=mysql_time_query($link,'SELECT sum(a.prepaid) as prepaid,count(a.id) as cc FROM booking_status_history AS a WHERE a.status=6 and  a.id_booking="'.htmlspecialchars(trim($_GET['id'])).'"');	
					 //echo('SELECT a.* FROM r_status AS a WHERE a.numer_status="'.$row1ss["status"].'" and a.id_system=13');
                if($result_status22->num_rows!=0)
                {  
                   $row_status22 = mysqli_fetch_assoc($result_status22);
...
echo'<div class="history_string"><div class="i_string">Аванс <span>(Кол-во платежей - '.$row_status22["cc"].')</span></div><div class="cost_string">'.rtrim(rtrim(number_format($row_status22["prepaid"], 2, '.', ' '),'0'),'.').'<span>₽</span></div></div>';
			 
		
	echo'<div class="history_string red_history"><div class="i_string">Долг по заявке составляет</div><div class="cost_string">'.rtrim(rtrim(number_format(($row_status21["cost"]-$row_status22["prepaid"]), 2, '.', ' '),'0'),'.').'<span>₽</span></div></div>';	
	
	
echo'<div style="margin-top: 30px;">
<div class="input_2018"><label>Аванс</label><input name="avanss1" value="" id="date_1w24" class="input_new_2018 required money_mask1 gloab avax" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"><hr class="tree"></div></div>
</div>';
...
{	
	...
	echo'<span class="koldd_w" style="display:none; width: 100%;"><div class="history_string ser_history"><div class="i_string">Остаток по заявке</div><div max_summ="'.($row_status21["cost"]-$row_status22["prepaid"]).'" class="cost_string gop_cost">'.rtrim(rtrim(number_format(($row_status21["cost"]-$row_status22["prepaid"]), 2, '.', ' '),'0'),'.').'<span>₽</span></div></div></span>';		
...
<div id="button_bron_booking_avaa1" class="save_button"><i>Внести аванс</i></div>
...
//выбираем какой вид оплаты
//вернул все деньги конец покупки
if($window_style==1)
{
...
$token=token_access_compile($_GET['id'],'bt_booking_select_end',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы	
...
echo'<div class="comme">Выберите необходимый вариант покупки по заявке. Частичная оплата предполагает авансовую форму оплаты, либо рассрочку.</div>';
	
	?>
<div class="radioselect" style="margin-top:20px;">

<div class="radio_material active_radio" radio_id="1"><div class="table r_mat"><div class="table-cell radio_wall"><div class="st_div_radio  radio_checkbox"><i></i></div><input class="rt_wall_radio radio_1" name="radio_r1" value="1" type="hidden"></div><div class="table-cell name_radio wallr1">Полная оплата (100%)</div></div></div>		

<div class="radio_material " radio_id="2"><div class="table r_mat"><div class="table-cell radio_wall"><div class="st_div_radio  radio_checkbox"><i></i></div><input class="rt_wall_radio radio_2" name="radio_r2" value="0" type="hidden"></div><div class="table-cell name_radio wallr1">Частичная оплата</div></div></div>		


<input value="1" id="radio_b" class="radio_b" name="radio_b_op" type="hidden">	
</div>			
	<?
...
<div id="no_rd" class="no_button"><i>Отменить</i></div>   	
<div id="button_bron_booking_prepaid1_x" class="save_button_no_icon"><i>Далее</i></div>
...
//составление секретного ключа формы
//соль для данного действия
$token=token_access_compile($_GET['id'],'bt_booking_avans_end22',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы	
...
if($num_results_t1==1)
{
	...
	echo'<div style="margin-top: 30px;"><div class="input_2018"><label>Номер заявки в ТО</label><input name="offers['.$i.'][operator_number]" value="" id="date_124" class="input_new_2018 required  gloab" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two">
	<div class="oper_name">'.$row_status222t["name"].'</div>
	</div></div>
</div>';	
...
echo'<div style="margin-top: 30px;">
<div class="input_2018"><label>Конечная цена туроператора</label><input name="offers[0][operator_cost]" value="" id="date_124" data-tooltip="сумма которую мы заплатили туропературу + коммиссия банка если она была" class="input_new_2018 required money_mask1 gloab" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"><hr class="tree"><div class="oper_name">'.$row_status222t["name"].'</div></div></div>
</div>';	

/*	
echo'<div style="margin-top: 30px;">
<div class="input_2018 active_in_2018"><label>Цена по чеку клиента</label><input name="offers[0][client_cost]" value="'.$row_score1["cost"].'" id="date_124" class="input_new_2018 required money_mask1 gloab" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"><hr class="tree"></div></div>
</div>';
*/	
...
echo'<div style="margin-top: 30px;" class="input_2018"><label>Конечная цена туроператора</label><input name="offers['.$i.'][operator_cost]" value="" data-tooltip="сумма которую мы заплатили туропературу + коммиссия банка если она была" id="date_124" class="input_new_2018 required money_mask1 gloab" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"><hr class="tree"><div class="oper_name">'.$row_score2["name"].'</div></div></div>
</div>';	
...
<div class="radio_material" radio_id="1"><div class="table r_mat"><div class="table-cell radio_wall"><div class="st_div_radio  radio_checkbox"><i></i></div><input class="rt_wall_radio radio_1" name="radio_doc1" value="0" type="hidden"></div><div class="table-cell name_radio wallr1">Договор клиенту распечатан и отдан</div></div></div>		
...
<div id="no_rd" class="no_button"><i>Отменить</i></div>   	
<div id="button_bron_booking_prepaid_end" class="save_button greii_d"><i>Купить</i></div>
