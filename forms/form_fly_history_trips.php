...
$result_t=mysql_time_query($link,'Select A.id from trips as A where A.visible=1 AND A.id="'.ht($_GET["id"]).'" and A.id_a_company IN ('.$id_company.')');
...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>История изменения времени и даты вылета по туру №'.$_GET["id"].'</span></h1>'; ?>
...
$result_mi=mysql_time_query($link,'SELECT a.* FROM trips_fly_history AS a WHERE a.id_trips="'.htmlspecialchars(trim($_GET['id'])).'"  order by a.datetime limit 1');
...
{	
	...
	echo'<div class="history_string_fly blue_history">
	   <div class="i_string">Время вылетов на момент бронирования тура<br> <span>'.time_stamp_dd1($row_mi["datetime"]).' - '.$payuser.'</span></div>
	...
	$result_mi=mysql_time_query($link,'SELECT a.* FROM trips_fly_history AS a WHERE a.id_trips="'.htmlspecialchars(trim($_GET['id'])).'" and not(a.datetime="'.$row_mi["datetime"].'") order by a.datetime');
