...
$result_t=mysql_time_query($link,'Select b.name_user from r_user as b where b.id="'.htmlspecialchars(trim($_GET['id'])).'"');
...
$token=token_access_compile($_GET['id'],'dell_users_xx',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Удаление Работника</span></h1>'; ?>
...
echo'<div class="comme">Вы точно хотите удалить работника - <b>'.$row_score["name_user"].'</b>?</div>';		
?>	
...
<div id="button_dell_users" class="save_button"><i>Удалить</i></div>
