...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Уточнение времени и даты вылета туриста</span></h1>'; ?>
...
echo'<input type="hidden" value="'.$token.'" name="tk">';		
	...
	$end_mi=$row_score["end_fly"];		
				...
				$result_mi=mysql_time_query($link,'SELECT a.* FROM booking_offers_fly_history AS a WHERE a.id_offers="'.htmlspecialchars(trim($_GET['id'])).'" order by a.datetime DESC limit 0,1');	
...
<div id="fly_booking_button1" class="save_button"><i>Сохранить</i></div>
