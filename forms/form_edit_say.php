<?php
//форма редактирования сообщения в общении клиента. Второе окно. поэтому forms.Js с этим окном не подгружаем
...
//может 
$result_t=mysql_time_query($link,'Select a.* from k_clients_commun as a where a.id="'.htmlspecialchars(trim($_GET['id'])).'" and a.visible=1');
...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111 js-form2" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Изменение сообщения №'.$_GET["id"].'</span></h1>'; ?>
...
<?
	...
	$time_task='';
	$date_task='';
	$result_status22task=mysql_time_query($link,'SELECT b.* FROM task_new AS b WHERE b.visible=1 and b.action=10 and b.status=0 and b.id_object="'.ht($_GET['id']).'" and b.id_user="'.ht($id_user).'"');	
...
$query_string='<div class="js-ssay" style="display:block;"><div class="add_say_form js-add-form-plus-task"><div class="form_left_say js-form_right_say1 form_say_'.$row_score["id_type_commun"].'"></div>
<div class="form_right_say">';

$query_string.='<div class="add_say_one">';
 ...
 $os_say = array();
	 ...
	 $result_work_zz=mysql_time_query($link,'Select a.* from k_clients_type_commun as a where a.visible=1 order by a.displayOrder');				 
		...
		$su_say=$row_score["id_type_commun"];
		   ...
		   $query_string.='</ul><input type="hidden" name="typesay_form2" id="typesay_form2" value="'.$su_say.'"></div></div>'; 



$query_string.='</div>	
<div class="add_say_two"><span class="js-form-gll_form2">
       <div class="div_textarea_say" >
	   <label>ваш комментарий</label>
			<div class="otziv_add">';
                 
                   
        $query_string.='<textarea style="height:20px" cols="10" rows="1" placeholder="" id="otziv_area_say2020" name="comment_b" class="di text_area_otziv no_comment_bill form_c45">'.$row_score["comment"].'</textarea>

 <script type="text/javascript"> 
	  $(function (){ 
$(\'#otziv_area_say2020\').autoResize({extraSpace : 10});
$(\'.form_c45\').trigger(\'keyup\');
...
$query_string.='</div>  <div class="add_sel_yes"><div class="center_vert right_task_ccb" style="width: 100%;"><div class="add_cff js-add-say1">изменить сообщение</div>
...
<div class="task_cb_exit js-exit-form-say1">
...
<script type="text/javascript">initializeTimer();</script>
 ...
 $(document).ready(function(){ 
  ...
  $('.js-box-modal-two').on("change keyup input click",'.task_cb',task_cb_head1);	 //кнопка задача
  $('.js-box-modal-two').on("change keyup input click",'.js-vibor-date1 span',js_vibor_date1);	//быстрые даты 
  $('.js-box-modal-two').on("change keyup input click",'.js-exit-form-say1',js_exit_form_sel1);  //кнопка отмена
	 
 $('.js-box-modal-two').on("change keyup input click",'.js-add-say1',add_say_yes1); //кнопка изменить
		...
		$(".drop").find("li").bind('click', dropli);
			 $('#typesay_form2').unbind('change', changesay1);
	 $('#typesay_form2').bind('change', changesay1);	 
