...
$token=token_access_compile($row_work_zz["id"],'bt_cash_close_buy',$secret);
?>
...
<div class="top_modal">
  ...
  <? 
  echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Закрытие смены (<i style="border-bottom: 2px solid #fd8080 !important; font-style: normal;">'.$row_work_zz["object_name"].'</i>)</span></h1>';
  ?>
...
if($status_admin==0)
{
    $query_string.='<div class="center_vert right_task_ccb" style="width: 100%; margin-top:20px;"><div class="add_cff_fin js-add-buy-cash-close-x">Подтвердить</div>';
}
