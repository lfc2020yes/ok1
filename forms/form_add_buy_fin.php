...
if ((!$role->permission('Финансы','A'))and($sign_admin!=1))
{
    goto end_code;	
}
...
$token=token_access_compile('2021','bt_finance_buy',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div class="top_modal">
  ...
  <? 
  echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Новая оперция</span></h1>';
  ?>
...
<?
echo'<form class="js-form-pay-finance" id="vino_xd_fiance_pay" style=" padding:0; margin:0;" method="post" enctype="multipart/form-data">';
...
$query_string.='<div class="left_drop list_2018 menu1_prime"><label class="">Вид операции<span>*</span></label><div class="select eddd zin_2019"><a class="slct" list_number="t3" data_src=""></a><ul class="drop js-visible-mt">';
...
if($result_8)
{
	...
	*/
		   ...
		   $query_string.='</ul><input type="hidden" class="gloab " name="vid_fin" id="vid_finance" value=""><div class="div_new_2018"><hr class="one"></div></div></div></div>';
...
if($status_admin==0)
{
    $query_string.='<div class="center_vert right_task_ccb" style="width: 100%; margin-top:20px;"><div class="add_cff_fin js-add-buy-finance-but-x">Провести</div>';
}
