...
$result_t=mysql_time_query($link,'Select a.id,a.id_users,a.id_reports,a.text from reports_answers as a where a.id="'.htmlspecialchars(trim($_GET['id'])).'"');
...
$token=token_access_compile($_GET['id'],'edit_answer_xx',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Изменение вопроса</span></h1>'; ?>
...
<div class="ok-block-input">
       ...
       <div class="div_textarea_otziv1" >
			<div class="otziv_add">
                   ...
                   <?
        echo'<textarea cols="10" rows="1" placeholder="Ваши вопросы по поводу рекламного тура" id="otziv_area_adaxx2" name="comment_b" class="di text_area_otziv no_comment_bill">'.$row_score["text"].'</textarea>';
				?>
            </div>      
</div>  
...
<div id="button_edit_answer" class="save_button"><i>Изменить</i></div>
