...
$result_uu = mysql_time_query($link, 'select * from trips_cancel where id_trips="' . ht($_GET['id']) . '"');
...
$token=token_access_compile($_GET['id'],'bt_edit_status_cancel',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div class="top_modal">
  ...
  <? 
  echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id_buy'])).'"><span>Изменение данных по аннуляции тура №'.$_GET["id"].'</span></h1>';
 ...
 as A where A.visible=1 and A.id_a_company IN ('.$id_group_company_list.') order by A.displayOrder');
    ...
    position: relative;" class="js-zindex js-vid-oper">	';
            ...
            $query_string.='<!--input start	-->		
<div style="margin-top: 30px;" class="rates_visible"><div class="input_2018"><label>ФПР туроператора</label><input name="com_proc" value="'.$row_uu["fpo"].'" id="date_124" class="input_new_2018 required   money_mask1 " autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"><hr class="'.$class_change.'"><div class="oper_name"></div></div></div>
</div>
<!--input end	-->';


            $query_string.='<!--input start	-->		
<div style="margin-top: 30px;" class="rates_visible"><div class="input_2018"><label>Общая стоимость тура для туриста при аннуляции</label><input name="com_rub" value="'.$row_uu["cost"].'" id="date_124" class="input_new_2018 required   money_mask1 " autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"><hr class="'.$class_change.'"><div class="oper_name"></div></div></div>
</div>
<!--input end	-->';
            ...
            if(($row_uu["id_refund"]!=0)and($row_uu["id_refund"]!=''))
            {
                $su_say=$row_uu["id_refund"];
            }
...
var dateParts1 = queryDate1.match(/(\d+)/g), realDate1 = new Date(dateParts1[0], dateParts1[1] -1, dateParts1[2]); 	 	 
            ...
            if($status_admin==0)
            {
                $query_string.='<div class="center_vert right_task_ccb" style="width: 100%; margin-top:20px;"><div class="add_cff_fin js-dell-cancel-trips-x">Сохранить</div>';
            }
...
отменить
 ...
 $(document).ready(function(){';
            ...
            if($row_uu["date_refund"]!='')
            {

                $query_string.='var date_v11=\''.$row_uu["date_refund"].'\';';
                $query_string.='$(\'#date_hidden_table_gr3300\').val(\''.$row_uu["date_refund"].'\');';
