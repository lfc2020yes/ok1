...
$result_t=mysql_time_query($link,'Select b.id,b.id_user,b.id_object from booking as b where b.id="'.htmlspecialchars(trim($_GET['id'])).'"');
...
if ((!$role->permission('Заявки','D'))and($sign_admin!=1))
{
    goto end_code;	
}
...
$token=token_access_compile($_GET['id'],'dell_booking_xx',$secret);
//составление секретного ключа формы
//составление секретного ключа формы
//составление секретного ключа формы
...
<div class="top_modal">
  ...
  <? echo'<h1 class="h111" mor="'.$token.'" for="'.htmlspecialchars(trim($_GET['id'])).'"><span>Удаление заявки</span></h1>'; ?>
...
echo'<div class="comme">Вы точно хотите удалить заявку <b>№'.$_GET["id"].'</b>?</div>';		
?>	
...
<div id="button_dell_booking" class="save_button"><i>Удалить</i></div>
