...
select * from(     
   (   
SELECT A.id,A.name FROM trips_country AS A where A.visible=1 AND LOWER(A.name) LIKE "'.$query.'%"   
)
UNION
(
...
SELECT A.id,A.name FROM trips_country AS A where A.visible=1 AND LOWER(A.name) LIKE "%'.$query.'%"
AND A.id NOT IN 
(SELECT A.id FROM trips_country A WHERE A.visible=1 AND LOWER(A.name) LIKE "'.$query.'%")
) 



) Z order by Z.name limit 0,30';  
} else
{
	$sql='Select a.name,a.id from trips_country as a WHERE a.visible=1 ORDER BY a.eye DESC,a.id limit 0,30';
}
