...
$result_uu = mysql_time_query($link, 'select A.id,A.id_user  from trips as A where A.id="' . ht($_POST['id']) . '"');
...
mysql_time_query($link,'INSERT INTO trips_status_history_new (id_trips,action_history,id_user,datetimes,edit,comment,comment_sys) VALUES ("'.ht($_POST['id']).'","15","'.ht($id_user).'","'.date("y.m.d").' '.date("H:i:s").'","1","'.ht($_POST['rate']).'","")');
...
/*
$result_8 = mysql_time_query($link,'SELECT A.*  FROM trips_status_history_new AS A WHERE  A.id="'.$ID_N.'"');
...
if($num_8!=0)
{
    ...
    $query_string_xx='<div rel_notibss="'.$row_8["id"].'" class="comm-trips-block new-say-com-t">';
