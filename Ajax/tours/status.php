...
//**************************************************
$result_url=mysql_time_query($link,'select A.id,A.status_admin,A.id_user from trips as A where A.id="'.ht($id).'"');
...
$result_uu = mysql_time_query($link, 'select A.id,A.id_user  from trips as A where A.id="' . ht($_GET['id']) . '"');
...
$st='';
$status_party=0;
if($row_list["status_admin"]==1)
{
    $st_old='к оплате';
	$st='оплачено';
	mysql_time_query($link,'update trips set status_admin="2" where id = "'.ht($id).'"');
	$status_party=2;
}

if($row_list["status_admin"]==0)

{
    mysql_time_query($link,'update trips set status_admin="1" where id = "'.ht($id).'"');
	$status_party=1;
    $st_old='на проверке';
	$st='к оплате';
}
if($row_list["status_admin"]==2)

{
    mysql_time_query($link,'update trips set status_admin="0" where id = "'.ht($id).'"');
    $status_party=0;
    $st='на проверке';
    $st_old='оплачено';
}
...
mysql_time_query($link,'INSERT INTO trips_status_history_new (id_trips,action_history,id_user,datetimes,edit,comment,comment_sys) VALUES ("'.ht($_GET['id']).'","26","'.ht($id_user).'","'.date("y.m.d").' '.date("H:i:s").'","0","Изменение статуса по туру №'.ht($_GET['id']).' → c \"'.$st_old.'\" на \"'.$st.'\"","")');




//отправляем уведомление владельцу тура если у него включено это уведомление
$text_not='"Изменение статуса по <a href="/tours/.id-'.ht($_GET['id']).'">туру №'.ht($_GET['id']).'</a> → c '.$st_old.' на '.$st.'.';
...
$aRes = array("debug"=>$debug,"status"   => $status_ee,"status_admin" =>  $status_party,"echo" => $st);
