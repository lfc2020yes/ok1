...
if ($num_results_ss_new != 0) {
    ...
    if(($array_param_new_ss[0] == 1) and ($array_param_new_ss[1] == 1)) {
        ...
        $comiss_ship = comiss_end_ship($row_uu11['id_trips'], $link);

        $proc_ship=partners_trips($row_uu11['id_trips'],$id_company,$link);
        mysql_time_query($link, 'update trips set
    ...
    where id = "' . ht($row_uu11['id_trips']) . '"');
        ...
        commission_add_ship($row_uu11['id_trips'],$comiss_ship,$proc_ship, $link);
