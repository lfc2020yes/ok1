...
//**************************************************
$result_t=mysql_time_query($link,'Select b.* from finance_operation as b where b.id="'.htmlspecialchars(trim($_GET['id'])).'" and b.visible=1 and b.id_a_company IN ('.ht($id_company).')');
...
if ($_GET["choice"]==1)
{
mysql_time_query($link,'update finance_operation set constant="1" where id = "'.ht($row_t["id"]).'"');
	
}
if ($_GET["choice"]==0)
{
    mysql_time_query($link,'update finance_operation set constant="0" where id = "'.ht($row_t["id"]).'"');
