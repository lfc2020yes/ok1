...
//**************************************************
//2 дня
if(!token_access_new($token,'bt_finance_buy',$id,"rema",2880))
{
...
mysql_time_query($link,'INSERT INTO finance_operation (
...
$result_uu = mysql_time_query($link, 'select A.*  from finance_operation as A where A.id="'.ht($new_id_payment).'"');
...
$aRes = array("debug"=>$debug,"status"   => $status_ee,"status_echo" => $status_echo,"blocks" => $dom,"for_id"=>ht($_POST["id"]),"id" => htmlspecialchars($ID_N),"error" =>  $stack_error,"income" => ht((int)$_POST["type_fin"]-1));
