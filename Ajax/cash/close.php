...
//**************************************************
//2 дня
if(!token_access_new($token,'bt_cash_close_buy',$id,"rema",2880))
{
...
mysql_time_query($link,'INSERT INTO cash_close_shift (
...
$text_not = 'Закрытие смены. Менеджер - '.$name_user.'. Сумма в кассе - '.number_format(((float)$_POST["summ"]), 2, '.', ' ').'₽.';

$user_send_new= array();
$user_send_new=array_merge(UserNotNumber(15,$link));
