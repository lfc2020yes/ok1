...
if(!token_access_new($token,'edit_answer_xx',$id,"s_form"))
{
...
$status_ee='ok';
mysql_time_query($link,'update reports_answers set text="'.htmlspecialchars($_GET['answer']).'" where id = "'.htmlspecialchars($_GET['id']).'"');
...
//$user_send_new=array_merge($hie->boss['4']);
   ...
   //создателя договора	
		$text_not='Вопрос «'.htmlspecialchars($row_score["text"]).'» по <a href="reports/'.$row_score["id_reports"].'/">Отчету №'.$row_score["id_reports"].'</a> был изменен на вопрос «'.htmlspecialchars($_GET['answer']).'».';
