...
if(!token_access_new($token,'bt_booking_avans_end',$id,"s_form"))
{
...
$disco=0;
	...
	$result_t2=mysql_time_query($link,'Select b.cost,b.id from booking_offers as b where b.id_booking="'.htmlspecialchars(trim($_POST['id'])).'" and b.status="2" and b.id="'.htmlspecialchars(trim($value['id'])).'"');
		   ...
		   $disco=$disco+$commission;
		    mysql_time_query($link,'update booking_offers set cost_operator="'.htmlspecialchars($cost_op).'",commission="'.$commission.'" where id = "'.htmlspecialchars($value['id']).'"');
   ...
   //создателя договора	
		$text_not='<a href="booking/'.$_GET['id'].'/">Заявка №'.$_POST['id'].'</a> была отмечена как забронированная. Вся рассрочка по данной заявке была получена. Прибыль составила '.rtrim(rtrim(number_format($disco, 2, '.', ' '),'0'),'.').' рублей';
