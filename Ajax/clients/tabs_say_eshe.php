...
//**************************************************
//**************************************************
$result_8 = mysql_time_query($link,'SELECT A.id,A.comment,B.var as ids, B.name, A.id_user, A.datetimes  FROM k_clients_commun AS A,k_clients_type_commun as B WHERE A.visible=1 and A.id_type_commun=B.id and A.id_client="'.htmlspecialchars(trim($id)).'" order by A.datetimes desc limit '.htmlspecialchars(trim($_GET['pg']*$_GET['all'])).','.htmlspecialchars(trim($_GET['all'])));
...
{	
  			  ...
  			  while($row_8 = mysqli_fetch_assoc($result_8)){
					  ...
					  $query_string_xx='<div rel_notib="'.$row_8["id"].'" class="say_cb">';
