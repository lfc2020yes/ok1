...
//********************************************************************
  ...
  DISTINCT A.id as ids,A.id_user,A.doc,hotel,A.place_name,A.date_start,A.id_contract,A.number_to,A.shopper,A.id_operator,A.id_shopper,A.date_end,A.id_country'.$sql_columb.'
...
if($num_results_t2!=0)
{
    ...
    for ($ksss=0; $ksss<$num_results_t2; $ksss++) {
        ...
        $data1 = array();
        include $url_system . 'contracts/code/block_doc1.php';
...
$titles = array("id","Номер договора", "Название тура","Даты отдыха","Клиент","Туроператор","№ заявки ТО","Статус","Менеджер");
