...
//ПАРТНЕРКА
//РОБОТ ПО ПРОВЕРКЕ ВОПРОСНЫХ ТУРОВ В КОТОРЫХ ЧТО-ТО МЕНЯЛАСЬ ПО ДАТАМ ВЫЛЕТА
//КАЖДЫЙ ЧАС
...
include_once $url_system.'ilib/lib_interstroi.php';
        ...
        $result_uu = mysql_time_query($link, 'select * from affiliates_trips_check');
  ...
  from trips as b where b.id="' . ht($row_uu["id_trips"]) . '" and not(b.id_affiliates=0)');
                ...
                if($flag==1)
                {
                    mysql_time_query($link, 'delete FROM affiliates_trips_check where id="' . ht($row_uu['id']) . '"');

                    mysql_time_query($link, 'update affiliates_history_trips set
                    
                    block="'.$block.'"
                    
                    where id_trips = "' . ht($row_uu['id_trips']) . '"');
...
mysql_time_query($link,'INSERT INTO cron_history (datetimes,script,message) VALUES ("'.date("Y-m-d").' '.date("H:i:s").'","affiliates_check_trips_1h_.php","'.ht($cron_message).'")');