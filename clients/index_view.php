...
if((isset($_POST['save_clients']))and($_POST['save_clients']==2))
{
...
$result_url=mysql_time_query($link,'select A.* from k_clients as A where A.visible=1 and A.id="'.htmlspecialchars(trim($_GET['id'])).'"');
        $num_results_custom_url = $result_url->num_rows;
        if($num_results_custom_url==0)
        {
           header404(5,$echo_r);
...
if($error_header!=404){ SEO('clients_view','','','',$link); } else { SEO('0','','','',$link); }
...
<?
	  ...
	  include_once $url_system.'template/top_clients_view.php';
...
{	
	...
	echo'<a class="svyz_client" href="clients/'.$row_list["id_client"].'/">
...
$(document).ready(function(){	
	...
	<div class="height_100vh" style="background-color: #e8eaed;">
	<div class="sourse_b">
	<?
...
{	
	...
	echo'<div class="sourse_2"><div class="input_2018 '.iclass_($rr1."_date_fly",$stack_offers,"error_2018").'"><label>Дата и город вылета<span>*</span></label><input '.$read.' name="sourse['.$rr1.'][date_fly]" value="'.ipost_($_POST['sourse'][$rr1]["date_fly"],$row_sourse["date_fly"]).'" id="date_fly_'.$rr1.'" class="input_new_2018 required form_offers_ " autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"></div></div></div>';				  
				  ...
				  if($row_sourse["start_fly"]!='0000-00-00 00:00:00')
				  {
					   $date_start=explode(" ",htmlspecialchars(trim($row_sourse["start_fly"])));
				   $date_start1=explode("-",htmlspecialchars(trim($date_start[0])));
				  ...
				  } else
				  {
					  ...
					  if($row_sourse["end_fly"]!='0000-00-00 00:00:00')
				  {	
	   $date_end=explode(" ",htmlspecialchars(trim($row_sourse["end_fly"])));
				   $date_end1=explode("-",htmlspecialchars(trim($date_end[0])));
	...
	echo'<div class="sourse_2"><div class="input_2018 '.iclass_($rr1."_start",$stack_offers,"error_2018").'"><label>Дата и время вылета</label><input '.$read.'  name="sourse['.$rr1.'][start]" value="'.ipost_($_POST['sourse'][$rr1]["start"],$startx).'" id="start_'.$rr1.'" class="input_new_2018 required time_input date_time_picker form_offers_ " autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"></div></div></div>';			

	echo'<div class="sourse_2"><div class="input_2018 '.iclass_($rr1."_end",$stack_offers,"error_2018").'"><label>Дата и время отлета</label><input '.$read.' name="sourse['.$rr1.'][end]" value="'.ipost_($_POST['sourse'][$rr1]["end"],$endx).'" id="end_'.$rr1.'" class="input_new_2018 required time_input date_time_picker form_offers_ " autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"></div></div></div>';			
...
{	
		...
		echo'<div class="sourse_2"><div class="input_2018 '.iclass_($rr1."_proc",$stack_offers,"erro_formi_2018").'"><label>Наш Возможный - %<span class="bluu">*</span></label><input '.$read.' name="sourse['.$rr1.'][proc]" value="'.ipost_($_POST['sourse'][$rr1]["proc"],$row_sourse["proc"]).'" id="proc_'.$rr1.'" class="input_new_2018 required  form_offers_ count_mask" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"></div></div></div>';	
...
{	
	...
	echo'<div class="sourse_2"><div class="input_2018"><label>Возможная скидка<span class="bluu">*</span></label><input '.$read.' name="sourse[SIDMIDS][discount]" value="" id="discount_SIDMIDS" class="input_new_2018 required form_offers_" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"></div></div></div>';		

	
	echo'<div class="sourse_2"><div class="input_2018"><label>Наш Возможный - %<span class="bluu">*</span></label><input '.$read.' name="sourse[SIDMIDS][proc]" value="" id="proc_SIDMIDS" class="input_new_2018 required count_mask form_offers_" autocomplete="off" type="text"><div class="div_new_2018"><hr class="one"><hr class="two"></div></div></div>';			
