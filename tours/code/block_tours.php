...
$task_cloud_block.='<div class="trips_block_global '.$new_sayx.' '.$cancel_class.'" id_trips="'.$row_8["id"].'"><span class="js-update-block-trips">
...
if (($role->permission('Туры','S'))or($sign_admin==1)) {
    if(($row_8["buy_clients"]==1)and($row_8["buy_operator"]==1)and($prof==1)) {
        $actv1 = '';


        $task_cloud_block .= '<div class="choice-radio js-status-trips-more"><div class="center_vert1"><i class="' . $actv1 . '"></i></div></div>';
    }
}
//впечатление по туру
if($row_8["comment"]!='') {
...
if($row_mi["end_fly"]!='0000-00-00 00:00:00') {
    ...
    if($d_day>0)
    {
        ...
        $task_cloud_block.='<textarea cols="10" rows="1" placeholder="" id="otziv_chat'.$row_8["id"].'" name="chat_text" class="di text_area_otziv no_comment_bill22_2 tyyo  
 gloab"></textarea>';
...
//связь с обращением
$result_uu_boo = mysql_time_query($link, 'select id_booking from trips where id="' . ht($row_8['id']) . '"');
$num_results_uu_boo = $result_uu_boo->num_rows;

if ($num_results_uu_boo != 0) {
    $row_uu_boo = mysqli_fetch_assoc($result_uu_boo);
    if($row_uu_boo['id_booking']!=0)
    {
        $task_cloud_block.='<div class="pass_wh_trips"><label>Обращение</label><a href="/preorders/.id-'.$row_uu_boo['id_booking'].'" class="obi-boo">Обращение №'.$row_uu_boo['id_booking'].'</a></div>';
    }
...
} else
{
    ...
    //$procr=floor(($sum_rub*100)/(float)$row_8["cost_client_exchange"]);
    $procr=round((($sum_rub*100)/$row_8["cost_client_exchange"]),4);

/*
    echo 'paid_client_rates- '.$sum_rub.' <br>';
    echo 'cost_client_exchange- '.$row_8["cost_client_exchange"].' <br>';
    echo 'procr- '.$procr.' <br>';
    echo 'procr- '.floor((690.94*100)/690.94).' <br>';
*/
}
...
} else
{
    ...
    {
        //что то еще не оплачено или клиентом или оператором
        //предполагаемая комиссия рассчитывается с валютой как будто остальное он отдает по такому же курсу что и при брони
        $task_cloud_block.='<span class="name-trips-opi">Предполагаемая Комиссия</span>';
        if($row_8["cost_operator_exchange"]!=0) {
...
$task_cloud_block.='<div class="mm_w-trips form004U">
    ...
    for ($i=0; $i<count($tabs_menu_x); $i++) {
        ...
        if($tabs_menu_x_id[$i]=="5")
        {
            ...
            //
            $result_uu22 = mysql_time_query($link, 'select count(A.id) as pp from trips_payment as A where A.id_trips="' . $row_8["id"] . '" and A.who=0 and A.visible=1');
            ...
            //
            $result_uu223 = mysql_time_query($link, 'select count(A.id) as pp from trips_payment as A where A.id_trips="' . $row_8["id"] . '" and A.who=1 and A.visible=1');
        ...
        if ($_GET['menu_id'] == $tabs_menu_x_id[$i]) {
            $task_cloud_block .= '<li class="tabs_004U active ' . $tabs_menu_x_js[$i] . '" id="' . $tabs_menu_x_id[$i] . '">' . $tabs_menu_x[$i] .$pay_string. '</li>';
        } else {
            $task_cloud_block .= '<li class="tabs_004U ' . $tabs_menu_x_js[$i] . '" id="' . $tabs_menu_x_id[$i] . '">' . $tabs_menu_x[$i] .$pay_string. ' </li>';
        }

    }

   if($row_8["status_admin"]==0) {
       $task_cloud_block .= '<li class="tabs_004U edit-li-tr"><a href="tours/' . $row_8["id"] . '/" class="edit-trips-all" data-tooltip="Изменить" ></a></li>';
   }
...
if($tabs_menu_x_visible[4]=="1") {
    ...
    if ($row_uuwe["status"] == 1) {
        ...
        $task_cloud_block .= '<li class="tabs_004U annul-li-tr" id="0"><a  class="edit-trips-all2" data-tooltip="Отменить Аннуляцию" ></a></li>';
    }
