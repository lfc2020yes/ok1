...
{
   $result=mysql_time_query($link,'select id from booking_attach where name="'.htmlspecialchars(trim($log)).'"');
...
if ((isset($_POST["id"]))and((is_numeric($_POST["id"])))) 
{
	  ...
	  { 
	     ...
	     { 
		   ...
		   $result_t=mysql_time_query($link,'Select a.* from booking as a where a.id="'.htmlspecialchars(trim($_POST['id'])).'"');
	       ...
	       {	
		     ...
		     if((($row_t["status"]==3)or($row_t["status"]==6)))
		     { 
	...
	mysql_time_query($link,'INSERT INTO booking_attach (id_booking,name,type,displayOrder,visible) VALUES ("'.htmlspecialchars(trim($_POST["id"])).'","","","0","0")');
	$ID_D=mysqli_insert_id($link);		   			 		

$uploaddir = $_SERVER["DOCUMENT_ROOT"].'/booking/scan/';
...
$allowedExts = array("pdf", "doc", "docx","jpg","jpeg","rtf","xlsx","xls"); 
...
if (move_uploaded_file($_FILES['thefile']['tmp_name'], $uploadfile)) {
 ...
 //загрузился
  mysql_time_query($link,'update booking_attach set visible="1",name="'.$name_imgs.'",type="'.$allowedExts[array_search($extension, $allowedExts)].'" where id = "'.$ID_D.'"');
	
} else
{
	mysqli_query($link,'delete FROM booking_attach where id="'.$ID_D.'"');
